...
export function debugTokens(accessToken?: string, refreshToken?: string) {
  ...
  if (!accessToken) {
    ...
    console.log(
      `  - Expires at: ${accessExpiration ? new Date(accessExpiration).toLocaleString() : 'Unknown'}`
    );
  ...
  if (!refreshToken) {
    ...
    console.log(
      `  - Expires at: ${refreshExpiration ? new Date(refreshExpiration).toLocaleString() : 'Unknown'}`
    );
